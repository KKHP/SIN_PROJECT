...
from ICMODEL import ICpropmodel as icmp;
import random
        ...
        if len(seeds) == 0:
            for node in range(1, novert + 1):
                ...
                if node in outdeg.keys():
                    s_n_influnece[node] = (s_n_influnece[node] + icmp.icpropmodel(graph, seeds + [node])) / novert
            ...
            marginal_profit = icmp.icpropmodel(graph, seeds + [prev_best]) - icmp.icpropmodel(graph, seeds)
